...
Android 应用：本地 YOLO 视觉 + 云端 DeepSeek 决策 + Shizuku 免 Root 触控 + 端云协同闭环。

> 需求来源：工作区 `猫娘助手.ds`（799 行完整需求书）
> 当前进度：**M1 完成 + M2 进行中**（骨架 / 主题 / 主界面 / 侧边菜单 / 知识库 / 任务 / 音乐 / 日志 / 设置）
> 版本：0.1.0 · 最低 Android 12（API 31）· 默认橘猫色
...
| `./gradlew test` / `assembleDebug` | ❌ **本机跑不了**，原因与解法见第五节 |
| 可在真机安装的 APK | ❌ 尚未产出（依赖上一条） |
