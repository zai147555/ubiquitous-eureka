...
| M3 | 悬浮窗、语音输入、无障碍服务（爬楼） | **部分完成**：悬浮窗聊天 ✅（可拖动气泡 + 迷你聊天窗，聊天记录与主界面同一份）；无障碍服务 ✅（只读语义树，**不做点击注入**）；语音**输入**未开始 |
| M4 | MediaProjection 抓屏 + YOLO 真机推理 + NCNN/Vulkan 接入 | ✅ **已完成**：NCNN 真机实测 234ms/帧（4.27 FPS）；抓屏服务已开放按需取帧（`acquireFrame`），Agent 工具可直接识别屏幕 |
...
| M15 | 插件系统（列表/已安装/已启用/已禁用/可更新/日志） | ✅ **已完成**（JSON 声明式插件：白名单策略 + 只读日志） |
| — | **Agent 工具链**（function calling：知识库检索 / 时间 / 网页正文 / 本地·服务端检测 / 播放音乐） | ✅ **已完成**（5 轮上限、动作类工具需用户确认、界面显示"正在调用 XX"） |
| — | **语音朗读**（微软官方 Edge TTS 端上直连 + 自动朗读 + 换音色） | ✅ **已完成**（零部署；失败自动回退系统 TTS；用官方语音时不做 RVC 变声，见代码注释） |
